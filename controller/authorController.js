...
exports.updateById = async (req, res) => {
  try {
    const updateAuthorById = await author.findOneAndUpdate(
      {id:req.params.id},
      req.body,
      {
        new: true,
    ...
    //const updatedAuthor=await author.find({id:req.params.id});
...
exports.deleteById = async (req, res) => {
  try {
    const deleteAuthorById = await author.deleteOne({id:req.params.id});
    if (!deleteAuthorById) {
      return res.status(STATUS_CODES.NOT_FOUND).json(MESSAGES.BOOK_NOT_FOUND);
    }
